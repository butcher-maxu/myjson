...
// JSONCC_ASSERT(...) macro will redirect to this. assert() macro is kept untouched.
#if defined(NDEBUG) || defined(_NDEBUG)
#   define JSONCC_REENABLE_NDEBUG
#   undef  NDEBUG
#   undef _NDEBUG
#endif
#include <stdio.h>
#include <cassert>
void jsoncc::assertion( const char *file, int line, const char *expression, bool result ) {
    if( !result ) {
        fprintf( stderr, "[JSONCC] expression '%s' failed at %s:%d -> ", expression, file, line );
        assert( 0 );
    }
}
#if defined(JSONCC_REENABLE_NDEBUG)
#   define  NDEBUG
#   define _NDEBUG
#endif
#include <cassert>
...
std::ostream& operator<<(std::ostream& stream, const jsoncc::Value& v) {
    using namespace jsoncc;
    if (v.is<Number>()) {
...
std::ostream& operator<<(std::ostream& stream, const jsoncc::Array& v) {
    stream << "[";
    jsoncc::Array::container::const_iterator
        it = v.values().begin(),
        end = v.values().end();
    while (it != end) {
        stream << *(*it);
...
std::ostream& operator<<(std::ostream& stream, const jsoncc::Object& v) {
    stream << "{";
    jsoncc::Object::container::const_iterator
        it = v.kv_map().begin(),
        end = v.kv_map().end();
    while (it != end) {
        jsoncc::stream_string(stream, it->first);
        stream << ": " << *(it->second);
        ++it;
        if (it != end) {
            stream << ", ";
...
namespace json {
    ...
    std::string tag( unsigned format, unsigned depth, const std::string &name, const jsoncc::Value &t) {
        std::stringstream ss;
        ...
        {
            default:
            case jsoncc::Value::NULL_:
                ss << "null";
                return ss.str() + ",\n";

            case jsoncc::Value::BOOL_:
                ss << ( t.bool_value_ ? "true" : "false" );
                return ss.str() + ",\n";

            case jsoncc::Value::ARRAY_:
                ss << "[\n";
            ...
            case jsoncc::Value::STRING_:
                ss << '\"' << escape_string( *t.string_value_ ) << '\"';
                return ss.str() + ",\n";

            case jsoncc::Value::OBJECT_:
                ss << "{\n";
...
} // namespace jsoncc::anon::json

} // namespace jsoncc::anon

std::string Object::json() const {
    using namespace json;

    jsoncc::Value v;
    v.object_value_ = const_cast<jsoncc::Object*>(this);
    v.type_ = jsoncc::Value::OBJECT_;

    std::string result = tag( jsoncc::JSON, 0, std::string(), v );
...
std::string Array::json() const {
    ...
    jsoncc::Value v;
    v.array_value_ = const_cast<jsoncc::Array*>(this);
    v.type_ = jsoncc::Value::ARRAY_;
