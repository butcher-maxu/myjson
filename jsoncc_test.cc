...
#   define TEST(...) JSONCC_ASSERT( __VA_ARGS__ );
    ...
    using namespace std;
    {
        ...
        string value;
        istringstream input(teststr); // istringstream用于从字符串中进行输入操作的类
    ...
    }
    {
        ...
        istringstream input(teststr);
        istringstream::iostate old_mask = input.exceptions(); //输入异常掩码
    ...
    {
        string teststr("asdfg");   // "asdfg" 不是需要解析度字符串格式 ""asdfg""  or "'asdfg'"
        istringstream input(teststr);
        Value v;
        TEST(!v.parse(input));
    }
...

    {
        ...
        a << 65.f;
        a << 65.0;
        a << jsoncc::Value( "hello world" );
        a << std::string("hello world");
        a << "hello world";

        TEST( jsoncc::Array().parse( a.json() ) );   // self-evaluation
        // cout << "a.json().c_str() = " << a.json().c_str() << endl;
...

    {
        ...
        jsoncc::Array a;
        a << jsoncc::Null();           // C++11: 'a << nullptr' is preferred
        a << 123;
        a << "hello \"world\"";
        a << "hello world";
        a << 3.1415;
...

    {
        ...
        TEST( jsoncc::Object().parse( o1.json() ) );  // self-evaluation
        // cout << "o1.json().c_str() = " << o1.json().c_str() << endl;
...
struct custom_type {}; 
    {
        ...
        o << "array" << a;
        o << "object" << jsoncc::Object("child", "object");
        o << "undefined" << custom_type();
        // cout << "o.json().c_str() = " << o.json().c_str() << endl;
        // if( verbose ) {
        ...
        TEST( jsoncc::Object().parse( o.json() ) );   // self-evaluation
...

    {
        ...
        // cout << "obj.json().c_str() = " << obj.json().c_str() << endl;
